...
foreach d $dbgs {
   ...
   if {![info exists clocks($name)]} {
      ...
      if {[llength $paths] > 0} {
         set epc [get_property ENDPOINT_CLOCK   [lindex $paths 0]]
         set spc [get_property STARTPOINT_CLOCK [lindex $paths 0]]
         if {[llength $epc] == 0} {
            set clocks($name) $spc
         } else {
            set clocks($name) $epc
...
foreach c [array names clock_list] {
   ...
   # Valid values for C_DATA_DEPTH are 1024 2048 4096 8192 16384 32768 65536 131072
   set_property       C_DATA_DEPTH     131072 [get_debug_cores $ila_inst]
...
write_debug_probes -force build/MEGA65-R3.ltx
